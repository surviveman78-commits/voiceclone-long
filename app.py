...
class VoxCPMDemo:
    ...
    @staticmethod
    def _split_long_text(text: str, max_chars: int = 220) -> list[str]:
        """Split long text into sentence-sized chunks for stable synthesis."""
        text = re.sub(r"\s+", " ", (text or "")).strip()
        if not text:
            return []

        # Keep Burmese and common Latin/CJK sentence punctuation with the
        # sentence before it. The whitespace is optional for Burmese text.
        sentences = [
            part.strip()
        ...
        for sentence in sentences:
            ...
            for word in words:
                ...
                # Burmese may have no spaces. Avoid an unbounded inference
                # call by using a hard fallback only when necessary.
                while len(current) > max_chars * 1.25:
    ...
    def generate_tts_audio(
        ...
        max_chars = int(os.getenv("VOXCPM_MAX_CHARS", "220"))
        chunk_pause_ms = int(os.getenv("VOXCPM_CHUNK_PAUSE_MS", "120"))
        chunks = self._split_long_text(final_text, max_chars=max_chars)
        logger.info("Long-form synthesis: %d chunk(s)", len(chunks))

        sample_rate = current_model.tts_model.sample_rate
        silence = np.zeros(int(sample_rate * chunk_pause_ms / 1000), dtype=np.float32)
        wav_parts: list[np.ndarray] = []

        for index, chunk in enumerate(chunks):
            logger.info("Generating chunk %d/%d: '%s...'", index + 1, len(chunks), chunk[:80])
            generate_kwargs = self._build_generate_kwargs(
                ...
                denoise=denoise,
                inference_timesteps=inference_timesteps,
                # A supplied seed remains reproducible, but each chunk gets
                # a different random trajectory to avoid repeated phrasing.
                seed=None if seed is None else seed + index,
            )
            wav = current_model.generate(**generate_kwargs)
            wav_parts.append(np.asarray(wav, dtype=np.float32).reshape(-1))
            if index < len(chunks) - 1:
                wav_parts.append(silence)

        wav = np.concatenate(wav_parts) if wav_parts else np.empty(0, dtype=np.float32)
        last_successful_seed = getattr(current_model.tts_model, "last_successful_seed", seed)
        return (sample_rate, wav, last_successful_seed)
